...
h = sigmoid([ones(size(a1,1),1) a1]*Theta2');

%K = zeros(m,1);
%
%for i=1:m
%
%K(i) = sum(-log(h(i)).*y_vector(i)-log(1-h(i)).*(1-y_vector(i)));
%
%end;
%
%J = sum(K)*(1/m);
...
%.*can't understand why it work but the above one can't
...
for i=1:size(Theta1,1)
    for j = 2:size(Theta1,2)
        sumOfTheta1+=Theta1(i,j)^2;
    end;
end;

for i=1:size(Theta2,1)
    for j = 2:size(Theta2,2)
        sumOfTheta2+=Theta2(i,j)^2;
    end;
end;
...
delta3 = h - y_vector(i,:);
delta2 = delta3*Theta2.*sigmoidGradient([1 [1 a1]*Theta1']);
